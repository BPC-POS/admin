...
const NotificationModal: React.FC<NotificationModalProps> = ({ isOpen, onClose }) => {
    ...
    return (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex justify-center items-center z-50 p-4 transition-opacity duration-300 ease-out">
            <div className="bg-white p-4 md:p-6 rounded-lg w-full max-w-lg shadow-xl relative max-h-[85vh] flex flex-col transform transition-all duration-300 ease-out scale-95 opacity-0 animate-fade-in-scale" style={{ animationName: 'fadeInScale', animationDuration: '0.3s', animationFillMode: 'forwards' }}> {/* Thêm animation */}
                <div className="flex justify-between items-center mb-4 pb-3 border-b border-gray-200 flex-shrink-0">
                    ...
                    <button
                        onClick={onClose}
                        className="text-gray-400 hover:text-gray-600 focus:outline-none text-3xl leading-none p-1 -m-1 rounded-full hover:bg-gray-100" // Cải thiện nút đóng
                        aria-label="Đóng"
                    >
                        ×
                    </button>
